...
searchRequestBtn.addEventListener("click", search);
personSearchInput.addEventListener("keydown", searchEnter);
  ...
  window
    ...
    .then(function (response) {
      if (response.status !== 200) {
        alert(
          "Произошла ошибка при получении ответа от сервера:" + response.message
        );
        return;
      }
    ...
    .then(function (response) {
      if (response.count == 0) {
        alert("Ничего не найдено. Попробуйте ввести другой запрос");
        return;
      }
